...
def get_user_challenge_progress(user_id, challenge_id):
    sql = text("SELECT COUNT(*) FROM UsersChallengeLogs WHERE user_id=:user_id AND challenge_id=:challenge_id AND completed=:completed")
    result = db.session.execute(sql, {"user_id": user_id, "challenge_id": challenge_id, "completed": True})
